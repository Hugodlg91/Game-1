...
class HorizontalSlider:
    ...
    def handle_event(self, event) -> bool:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if self.rect.collidepoint(event.pos):
                self.dragging = True
                self._update_value(event.pos[0]) # Use X coordinate
                changed = True
    ...
    def _update_value(self, mouse_x):
        """Calculate value based on mouse X position (left is 0.0, right is 1.0)."""
        # Relative X from left of rect
    ...
    def draw(self, surface):
        ...
        filled_w = int(self.rect.width * self.value)
        
        # Fill rect (left to right)
        fill_rect = pygame.Rect(
            self.rect.x, 
            self.rect.y, 
            filled_w, 
        ...
        # Optional: Draw a "handle" line at the right of the fill
        handle_x = self.rect.x + filled_w
        pygame.draw.line(surface, (255, 255, 255), (handle_x, self.rect.y), (handle_x, self.rect.bottom), 2)

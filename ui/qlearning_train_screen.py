"""Visual Q-Learning trainer screen.

Runs training incrementally via q_learning_agent.Trainer (one step per update).
"""
...
class QLearningTrainScreen(Screen):
    def __init__(self, manager):
        ...
        self.trainer = q_learning_agent.Trainer(episodes=1000, alpha=0.1, gamma=0.9, epsilon=0.2)
        self.paused = False
    ...
    def draw(self):
        ...
        lines = [f"Episode: {stats['episode']}/{stats['episodes']}", f"Step: {stats['step']}", f"ε={stats['epsilon']}", f"Qsize={stats['qsize']}"]
